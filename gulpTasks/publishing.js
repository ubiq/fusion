...
gulp.task('upload-binaries', (cb) => {
    ...
    got(`https://api.github.com/repos/ubiq/fusion/releases?access_token=${GITHUB_TOKEN}`, { json: true })
        ...
        if (draft.draft === true) {
            ...
            .then(() => {
                if (draft.body && checksums) {
                    got.patch(`https://api.github.com/repos/ubiq/fusion/releases/${draft.id}?access_token=${GITHUB_TOKEN}`, {
                        body: JSON.stringify({
                            tag_name: `v${version}`,
                            body: `${draft.body}\n\n## Checksums\n\`\`\`\n${checksums.join('')}\`\`\``
                        })
