...
const applicationName = (options.wallet) ? 'Ubiq Wallet' : 'Fusion';
...
gulp.task('bundling-interface', (cb) => {
    ...
    if (type === 'wallet') {
        if (options.walletSource === 'local') {
            ...
            console.log(`Pulling https://github.com/ubiq/meteor-dapp-wallet/tree/${options.walletSource} "${options.walletSource}" branch...`);
            bundle(`&& cd ../dist_${type} \
                && git clone --depth 1 https://github.com/ubiq/meteor-dapp-wallet.git \
