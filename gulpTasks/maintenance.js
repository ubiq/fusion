...
gulp.task('update-nodes', cb => {
  ...
  got('https://api.github.com/repos/ubiq/go-ubiq/releases/latest', {
    json: true
  })
    .then(response => {
    ...
    .then(tagName => {
      ...
      // Compare to current gubiq version in clientBinaries.json
      if (cmp(latestGethVersion, localGethVersion)) {
        geth.version = latestGethVersion;

        // Query commit hash (first 8 characters)
        got(
          `https://api.github.com/repos/ubiq/go-ubiq/commits/${tagName}`,
          { json: true }
        )
          .then(response => {
